...
function Dashboard({ reportUrl }) {
    const [dashboardState, setDashboardState] = useState({
        severityData: [],
        patchSummaryData: [],
        topVulnerableResourcesData: [],
        vulnerabilityAgeDistribution: [],
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                const data = await DashboardData(reportUrl);
                setDashboardState(prev => ({ ...prev, ...data, isLoading: false }));
            } catch (error) {
    ...
    const { severityData, patchSummaryData, topVulnerableResourcesData, vulnerabilitiesByType, vulnerabilityAgeDistribution, noVulnerabilityData, isLoading, error } = dashboardState;

    if (isLoading) {
        return (
            <div style={{ padding: '20px', textAlign: 'center', color: '#666' }}>
                Carregando dashboard de vulnerabilidades...
            </div>
        );
    }
    ...
    if (noVulnerabilityData) {
        return (
            <div style={{ 'margin': '15px', textAlign: 'center', color: '#666' }}>
                Nenhum dado de vulnerabilidade encontrado.
            </div>
        );
    }
